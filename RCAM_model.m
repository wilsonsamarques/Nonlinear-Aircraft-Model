...
%NOTE: We will denife Ib and invIb later
...
g = 9.81;                         % Gravitational acceleration (m/m^2)
...
a0 = 15.212;                      % Coefficient of alpha^0 (different from RCAM documentation)
alpha_switch = 14.5*(pi/180);      % alpha where lift slope does from linear to non-linear


%-----------------------1. CONTROL LIMITS/SATURATION ---------------------%
% Note that these can alternatevely be enforced in Simulink.
% u1min = -25*pi/180;
% u1max = 25*pi/180;
% 
% u2min = -25*pi/180;
...
%Also defines the vectors wbe_b and V_b
wbe_b = [x4;x5;x6];
V_b = [x1;x2;x3];
...
% Calculate the actual dimension forces. These are in F_s (stability axis)
